...
def build_sentinel_tokens(session: requests.Session, device_id: str, flow: str, *, include_so: bool = False) -> SentinelTokenBundle:
    return _build_sentinel_tokens(
        session,
        device_id,
        flow,
...
class PlatformRegistrar:
    ...
    def _authorize_continue(self, email: str, index: int) -> None:
        ...
        payload = {"username": {"value": email, "kind": "email"}, "screen_hint": "signup"}
        resp, error = request_with_local_retry(self.session, "post", url, json=payload, headers=headers, allow_redirects=True, verify=False)
        if _is_cloudflare_challenge(resp):
            bundle = self._refresh_cloudflare_clearance(auth_base, index)
            if bundle is None:
                raise RuntimeError(_cloudflare_block_message(resp, reason=self.clearance_failure_reason))
        ...
        if resp is None or resp.status_code not in (200, 302):
            ...
            detail = f", detail={json.dumps(data, ensure_ascii=False)}" if data else f", {_response_debug_detail(resp)}"
            raise RuntimeError(error or f"authorize_continue_http_{getattr(resp, 'status_code', 'unknown')}{detail}")
        step(index, "提交邮箱 continue 完成")
    ...
    def _create_account(self, name: str, birthdate: str, index: int) -> None:
        ...
        sentinel = build_sentinel_tokens(self.session, self.device_id, "oauth_create_account", include_so=True)
        if not sentinel.so_token:
            raise RuntimeError("OpenAI-Sentinel-SO-Token 生成失败")
        headers["OpenAI-Sentinel-Token"] = sentinel.sentinel_token
        headers["OpenAI-Sentinel-SO-Token"] = sentinel.so_token
        if sentinel.oai_sc:
            self.session.cookies.set("oai-sc", sentinel.oai_sc, domain=".openai.com")
            self.session.cookies.set("oai-sc", sentinel.oai_sc, domain=".auth.openai.com")
        step(
            index,
            f"Sentinel SDK {sentinel.sdk_version or '?'} token_len={len(sentinel.sentinel_token)} so_generated={bool(sentinel.so_token)} so_len={len(sentinel.so_token)} so_required={sentinel.sentinel_req_so_required}",
        )
        headers = _headers_with_clearance(headers, url, self.proxy, self.clearance_user_agent)
