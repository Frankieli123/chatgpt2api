...
class RegisterProxyRuntimeTests(unittest.TestCase):
    ...
    def test_create_account_sends_sentinel_and_so_headers(self):
        ...
        sentinel = SimpleNamespace(
            sentinel_token='{"p":"proof","t":"","c":"challenge","id":"device","flow":"oauth_create_account"}',
            so_token="so-token",
            oai_sc="0challenge",
            sdk_version="20260219f9f6",
            sentinel_req_so_required=True,
        )
        ...
        with patch.object(openai_register, "proxy_settings", fake_proxy), patch.object(
            ...
            "create_session",
            return_value=FakeSession(),
        ), patch.object(openai_register, "build_sentinel_tokens", return_value=sentinel), patch.object(
            openai_register,
        ...
        self.assertEqual(headers["OpenAI-Sentinel-Token"], sentinel.sentinel_token)
        self.assertEqual(headers["OpenAI-Sentinel-SO-Token"], "so-token")

    def test_create_account_stops_when_so_token_is_missing(self):
        sentinel = SimpleNamespace(
            sentinel_token='{"p":"proof","t":"","c":"challenge","id":"device","flow":"oauth_create_account"}',
            so_token="",
            oai_sc="0challenge",
        ...
        ), patch.object(openai_register, "request_with_local_retry") as request_mock:
            registrar = openai_register.PlatformRegistrar(proxy="")
            with self.assertRaisesRegex(RuntimeError, "OpenAI-Sentinel-SO-Token"):
                registrar._create_account("Test User", "2000-01-01", 1)
